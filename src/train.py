...
def train_guardrail(config):
    ...
    mlp_optimizer = torch.optim.Adam(mlp_probe.parameters(), lr=lr)
    ...
    for epoch in range(epochs):
        ...
        for i in tqdm(range(0, max_batches, batch_size), desc=f"Epoch {epoch+1}/{epochs}"):
            ...
            log_prob_fake = guardrail(batch_fake, mpe_training=False)
            loss_neg = torch.relu(static_margin + log_prob_fake.mean() - log_prob_true.mean())
            
            pc_total_loss = (alpha * loss_pos) + ((1 - alpha) * loss_neg)
            ...
            # ------------------------------------------------
            # 2. Train MLP Probe (Discriminative Baseline)
            # ------------------------------------------------
            mlp_optimizer.zero_grad()
            with torch.no_grad():
                emb_true = guardrail._get_llm_embeddings(batch_true)
                emb_fake = guardrail._get_llm_embeddings(batch_fake)
                
            logits_true = mlp_probe(emb_true)
            logits_fake = mlp_probe(emb_fake)
            
            logits = torch.cat([logits_true, logits_fake], dim=0)
            labels = torch.cat([torch.zeros_like(logits_true), torch.ones_like(logits_fake)], dim=0)
            
            mlp_loss = mlp_criterion(logits, labels)
            mlp_loss.backward()
            mlp_optimizer.step()
        ...
        if avg_total < best_pc_loss:
            ...
            # (MLP can stay as state_dict since its architecture is completely static)
            torch.save(mlp_probe.state_dict(), mlp_save_path)
